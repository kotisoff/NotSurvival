...
function exp.get_xp(pid)
  return player_data.get_data(pid).xp;
end
...
function exp.give(pid, amount)
  ...
  player_data.get_data(pid).xp = xp + amount;
end
...
function exp.drain(pid, amount)
  ...
  player_data.get_data(pid).xp = xp;
  return true;
end
...
function exp.drain_lvl(pid, amount)
  ...
  player_data.get_data(pid).xp = exp.calc_total(lvl);
  return true;
end

function exp.set_xp(pid, val)
  player_data.get_data(pid).xp = val;
end

function exp.set_lvl(pid, val)
  local total = exp.calc_total(val);
  player_data.get_data(pid).xp = total;
end
